...
public class UserService {
    ...
    public void addBooksToCart(Integer id, String token) {
        ...
        Book bookToAddOld = bookRepo.findById(id).get();
        if(bookToAddOld == null){
        ...
        Optional<BookOrder> bookTwo = cart.books.stream()
                .filter(b -> b.getBookAuthor().equals(bookToAddOld.getBookAuthor()))
                .filter(b -> b.getBookName().equals(bookToAddOld.getBookName()))
                .findAny();
        if(bookTwo.isPresent()) {
           throw new Exception("Book is already in cart");
        }
        if(bookTwo.isEmpty()) {
            bookToAddOld.setQuantity(bookToAddOld.getQuantity()-1);
            bookRepo.save(bookToAddOld);
    ...
    public void removeBooksFromCart(Integer id, String token) {
        ...
        BookOrder bookFound = bookOrderRepo.findById(id)
                .orElseThrow(() -> new Exception("Book not found."));
        Optional<Book> bookInLibrary = bookRepo.findByBookNameAndBookAuthor(bookFound.getBookName(), bookFound.getBookAuthor());
        if(bookInLibrary.isEmpty()) {
            throw new Exception("Book not bought from Store.");
        }
        bookInLibrary.get().setQuantity(bookInLibrary.get().getQuantity() + bookFound.getCountOfBook());
        bookRepo.save(bookInLibrary.get());
